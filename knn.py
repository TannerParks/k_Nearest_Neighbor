...
def main():
    ...
    # and have expected outputs given. All asserts should pass.
    ############################################################
    ...
    example_train_y = np.array([[0], [1], [1], [1], [0], [1]])                  # What each x is classified as
    ...
    for k in [1, 3, 5, 7, 9, 99, 999, 8000]:
        ...
        predicted_y = predict(train_X, train_y, train_X, k)
        train_acc = compute_accuracy(train_y, predicted_y)

        #######################################
        # TODO Compute 4-fold cross validation accuracy
        #######################################
        # Run it through cross validation
        # Use accuracy/variance from the cross validation function
...
def get_nearest_neighbors(example_set, query, k):
    """Gets the points from the example_set that's closest in distance to the query."""
    # TODO
    distances = np.linalg.norm(example_set - query, axis=1)
    idx_of_nearest = distances.argsort()[:k]  # Includes the k lowest values in ascending order by the index
...
def knn_classify_point(examples_X, examples_y, query, k):
    """Classifies the query based on the classes of the k-nearest neighbors."""
    # TODO

    # Find nearest neighbors for each query
    nearest_neighbors = get_nearest_neighbors(examples_X, query, k)

    # Find classification of each of the neighbors
    classifications = [examples_y[nearest_neighbors[i]] for i in range(len(nearest_neighbors))]

    # Flatten the classification list so bincount works easier with it
    flat_classes = np.concatenate(classifications)

    # Find what class (0, 1) appears the most for each query
    predicted_label = np.bincount(flat_classes.astype(int)).argmax()    # astype makes sure it's an int and not a float
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    ...
    for i in range(num_folds):
        testing_X = np.vstack(split_X[i])   # Arrange data into testing and training groups
        testing_y = np.vstack(split_y[i])
        training_X = np.vstack(split_X[:i] + split_X[i + 1:])
        training_y = np.vstack(split_y[:i] + split_y[i + 1:])
        pred = predict(training_X, training_y, testing_X, k)    # add to list to check accuracy with true y later
        predicted_y.append(compute_accuracy(testing_y, pred))

    avg_val_acc = np.mean(predicted_y)
    varr_val_acc = np.var(predicted_y)
